...
esnli_label_mapping = {0: 'entailment.', 1: 'neutral.', 2: 'contradiction.'} 
...
def relabel(row):
    if row['label'] == "NOT ENOUGH INFO" or row['label'] == "NOT_ENOUGH_INFO":
        return 1
...
#                        help="run name for the experiment")
    parser.add_argument("--project_name", default='few-shot-correction', type=str, required=False, 
...
#                        help="dataset file")
    ...
    parser.add_argument("--num_classes", default= 3, type=int, required=False, 
    ...
    parser.add_argument("--early_stopping_patience", default= 1, type=int, required=False, 
    ...
    run_folder = "/".join((args.model_name, args.source_dataset_name, args.dataset_name, args.sample_selection, 'st'+str(args.n_shots)))
    run_name = "+".join((run_folder, args.explanation_sep, args.io_format, 'sd'+str(args.seed), 'ssd'+str(args.sample_seed)))
...
#             df_train = pd.read_json('e-fever-8-shots.json', lines=True)
        ...
        if args.sample_selection == 'mr':
            df_train = pd.read_csv('chatgpt-explanations.csv', sep=';')
            df_train['explanation_1'] = df_train['explanation_2']
            df_train['explanation_2'] = ''
            df_train['explanation_3'] = ''
#             df_train['label'] = df_train.apply(relabel, axis=1)
            pa_tab= pa.Table.from_pandas(df_train)
...
#             dataset['train'] = datasets.Dataset(pa_tab)  
        ...
        if args.sample_selection == 'mr-accept-fastvotek':
            df_train = pd.read_json('/few-shot/icl-selective-annotation/mr-efever_accept_fastvotek_'+str(args.n_shots)+'.json', lines=True)
#             df_train['explanation_1'] = df_train['explanation']
            pa_tab= pa.Table.from_pandas(df_train)
            dataset['train'] = datasets.Dataset(pa_tab)  
...
#             df_train = pd.read_json('fever_random_sampling_train.json', lines=True)
            df_train = pd.read_csv('fever_random_sampling_train.csv')
    #         df_train = pd.read_json('e-fever-8-shots.json', lines=True)
    #         df_train = pd.read_json('fever-to-german.json', lines=True)
#             df_train = df_train.loc[df_train['explanation_1'] != 'empty']
            df_train = df_train[['premise','hypothesis','label', 'explanation_1']]
#             df_train['explanation_2'] = ''
#             df_train['explanation_3'] = ''
#             df_train['label'] = df_train.apply(relabel, axis=1)
            pa_tab= pa.Table.from_pandas(df_train)
        ...
        if args.sample_selection == 'accept-fastvotek':
            df_train = pd.read_json('/few-shot/icl-selective-annotation/mr-fever_accept_fastvotek_'+str(args.n_shots)+'.json', lines=True)
            pa_tab= pa.Table.from_pandas(df_train)
            dataset['train'] = datasets.Dataset(pa_tab) 
...
#     if args.sample_selection == 'random': as default, we always select validation sets according to a random seed
    ...
    if args.sample_selection == 'random' or args.sample_selection == 'retrieval':  
        
        ## this shuffle is for sample selection
...
#         dataset['validation'] = dataset['validation'].shuffle()
    ...
    for split in ["train", "validation"]:
        ...
        for label in [0,1,2]:
            if args.num_classes==2 and label == 1:
                continue
...
#     print('length of copy:', len(original_data_splits["train"]))
    ...
    #   TRAIN NOW   #
    #################
#     max_steps = args.max_steps * args.n_shots,
...
#         save_total_limit = 1,
#         logging_steps=1,
#         save_steps = args.max_steps * int(np.log2(args.n_shots)), # Evaluation and Save happens every 50 steps
...
#     save_path = trainer.state.best_model_checkpoint
    save_path = os.path.join(args.save_model_path, run_name)
...
# 
    ...
    if args.dataset_name == 'fever-eval-full':
    ...
    if args.dataset_name == 'fever-eval-2class':
        df_fm2 = pd.read_json('/few-shot/LOREN/data/fever/baked_data/bert_eval.json', lines=True)
        df_fm2['hypothesis'] = df_fm2['claim']
        df_fm2['premise'] = [' '.join([x[2] for x in y]) for y in df_fm2['evidence']]
        df_fm2 = df_fm2[['premise', 'hypothesis', 'label']]
        df_fm2['explanation_1'] = [""]*len(df_fm2)
        tmp = df_fm2.loc[df_fm2['label']=='NOT ENOUGH INFO']
    ...
    if args.dataset_name == 'climate-fever-combined':
        df_fm2 = pd.read_json('/few-shot/prompt-engineering/feb/data/climate-fever/climate-fever-dataset-r1.jsonl', lines=True)
        df_fm2['hypothesis'] = df_fm2['claim']
        df_fm2['label'] = df_fm2['claim_label']
        df_fm2['premise'] = [' '.join([x['evidence'] for x in y]) for y in df_fm2['evidences']]
        df_fm2 = df_fm2[['premise', 'hypothesis', 'label']]
        tmp = df_fm2.loc[df_fm2['label']=='DISPUTED']
        df_fm2 = df_fm2[ ~df_fm2.index.isin(tmp.index) ]
        df_fm2['explanation_1'] = [""]*len(df_fm2)

    if args.dataset_name == 'climate-fever-separate':
        cf = pd.read_json('/few-shot/prompt-engineering/feb/data/climate-fever/climate-fever-dataset-r1.jsonl', lines=True)
        claim = []
        evidence = []
        label = []
        for item, row in cf.iterrows():
        ...
        df_fm2 = pd.DataFrame(dict_prepare, columns = ['hypothesis','premise', 'label'])
        df_fm2['explanation_1'] = [""]*len(df_fm2)
    ...
    if args.dataset_name == 'e-fever-train':
        fever_dev = pd.read_json('/few-shot/LOREN/data/fever/baked_data/bert_train.json', lines=True)
        pub_dev = pd.read_json('/few-shot/prompt-engineering/efever/efever_train_set.jsonl', lines=True)
        df_fm2 = pd.merge(fever_dev, pub_dev, on='id', how='inner')
        df_fm2 = df_fm2.drop(columns=['id', 'evidence'])
        df_fm2 = df_fm2.rename(columns={"retrieved_evidence": "premise", "claim": "hypothesis", 'summary': 'explanation_1'})
        ## next rows are to filter samples with explanations repeat hypothesis while the label is not entailment
...
#         df_fm2 = df_fm2[ ~df_fm2.index.isin(tmp.index) ]
    ...
    if args.dataset_name == 'vitaminc-full':
...
#         df_fm2 = df_fm2.sample(frac=1, random_state=args.seed)[:1000]
    ...
    if args.dataset_name == 'vitaminc-2class':
        df_fm2 = pd.read_json('/few-shot/qafact/data/vitaminc/processed_test.json', lines=True)
        # print(df_fm2)
        df_fm2['hypothesis'] = df_fm2['claim']
        df_fm2['premise'] = df_fm2['gold_evidence']
...
#         df_fm2 = df_fm2[:20]
    ...
    if args.dataset_name == 'scifact':
        df_fm2 = pd.read_json('/few-shot/scifact/scifact_dev_2class.json', lines=True)
        df_fm2 = df_fm2.rename(columns={"evidence": "premise", "claim": "hypothesis"})
        df_fm2 = df_fm2.drop(columns=['id'])
        df_fm2['explanation_1'] = [""]*len(df_fm2)
...
#     answer_string = ' '.join(answer_string.split())
    ...
    if os.path.isfile(save_path+'/optimizer.pt'):
        os.remove(save_path+'/optimizer.pt')
        os.remove(save_path+'/pytorch_model.bin')
    else:    ## Show an error ##
